...
class CK3LogParser:
    ...
    def __init__(self, root: tk.Tk):
        ...
        self.translations = {
            "ru": {
                ...
                # Игровой набор
                "playset": "Игровой набор (Playset)",
                "playset_only": "Игровой набор",
                "browse_playset": "Обзор",
                "check_conf_playset": "🧩 Проверить конфликты (Playset)",
                "playset_load_error": "⚠️ Ошибка чтения файла набора: {err}",
                "playset_not_found": "⚠️ Файл набора не найден.",
                "playset_limited": "🎯 Проверка ограничена игровым набором ({count} модов).",                
            ...
            "en": {
                ...
                # Playset
                "playset": "Playset File",
                "playset_only": "Playset",
                "browse_playset": "Browse",
                "check_conf_playset": "🧩 Check Conflicts (Playset)",
                "playset_load_error": "⚠️ Playset file read error: {err}",
                "playset_not_found": "⚠️ Playset file not found.",
                "playset_limited": "🎯 Check limited to selected playset ({count} mods).",
        ...
        self.localmods_entry = None
        
        self.status_var = tk.StringVar(value="Ready")
        self.editor_choice = tk.StringVar(value="vscode")  # по умолчанию VS Code
        
        # Настройки набора (Playset)
        self.playset_entry = None
        self.playset_path = tk.StringVar(value="")
        self.playset_enabled = tk.BooleanVar(value=False)
    ...
    def _setup_ui(self):
        ...
        # ─── Новый блок: Local Mods Folder ───
        ttk.Label(cfg, text=t("localmods")).grid(row=2, column=0, sticky=tk.W)
        self.localmods_entry = ttk.Entry(cfg, width=70)
        self.localmods_entry.grid(row=2, column=1, padx=5)
        ttk.Button(cfg, text=t("browse"), command=self._browse_localmods).grid(row=2, column=2)

        # ─── Новый блок: Игровой набор (Playset) ───────────────────
        self.playset_check = ttk.Checkbutton(
            cfg, text=t("playset_only"), variable=self.playset_enabled, command=self._save_config)
        self.playset_check.grid(row=3, column=0, sticky=tk.W)

        self.playset_entry = ttk.Entry(cfg, width=70, textvariable=self.playset_path)
        self.playset_entry.grid(row=3, column=1, padx=5)
        ttk.Button(cfg, text=t("browse_playset"), command=self._browse_playset).grid(row=3, column=2)

        editor_frame = ttk.LabelFrame(cfg, text=t("editor"), padding=(5, 2))
        editor_frame.grid(row=4, column=0, columnspan=3, sticky=tk.W, pady=(8, 0))
        ...
        self.mt_var = tk.BooleanVar(value=self.use_multithread)
        self.mt_check = ttk.Checkbutton(
            cfg, text=t("multithread"),
            variable=self.mt_var,
            command=self._toggle_multithread
        )
        self.mt_check.grid(row=5, column=0, sticky=tk.W, pady=(5, 0))
    ...
    def _toggle_multithread(self):
        """Переключает режим многопоточности (вкл./выкл.)"""
        try:
            # обновляем флаг по состоянию чекбокса
            self.use_multithread = bool(self.mt_var.get())
            # сохраняем изменение в config.json
            self._save_config()

            # логируем действие (если лог уже инициализирован)
            msg = f"⚙️ Multithreading {'enabled' if self.use_multithread else 'disabled'} via config"
            if hasattr(self, "log_text") and self.log_text:
                self._log(msg)
            else:
                print(msg)
        except Exception as e:
            # защищаемся от ошибок GUI во время инициализации
            print(f"[toggle_multithread] Error: {e}")
    def _redraw_ui(self):
        ...
        saved_local = self.localmods_entry.get() if self.localmods_entry else ""
        saved_editor = self.editor_choice.get() if hasattr(self, "editor_choice") else "vscode"
        saved_playset = self.playset_path.get() if hasattr(self, "playset_path") else ""
        saved_playset_flag = self.playset_enabled.get() if hasattr(self, "playset_enabled") else False
        ...
        self.localmods_entry.delete(0, tk.END)
        self.localmods_entry.insert(0, saved_local)
        self.editor_choice.set(saved_editor)
        self.playset_path.set(saved_playset)
        self.playset_enabled.set(saved_playset_flag)
        ...
        try:
            ...
            if config_path.exists():
                with open(config_path, "r", encoding="utf-8") as f:
                    cfg = json.load(f)
                if cfg.get("use_multithread") is not None:
                    self.use_multithread = bool(cfg["use_multithread"])
                else:
                    self.use_multithread = True  # по умолчанию включено                   
                ...
                if cfg.get("localmods_path"):
                    self.localmods_entry.insert(0, cfg["localmods_path"])
                if cfg.get("playset_path"):
                    self.playset_path.set(cfg["playset_path"])
                if cfg.get("playset_enabled") is not None:
                    self.playset_enabled.set(cfg["playset_enabled"])
            ...
            self._log("⚠️ config.json повреждён, будет пересоздан.")
            self._save_config()
        except Exception as e:
            self._log(self.i18n("config_load_error").format(err=e))
        if hasattr(self, "mt_var"):
            self.mt_var.set(self.use_multithread)            
    def _save_config(self):
        cfg = {
            "use_multithread": self.use_multithread,
            "logs_path": self.logs_entry.get().strip(),
            "workshop_path": self.workshop_entry.get().strip(),
            "localmods_path": self.localmods_entry.get().strip(),  # 🟢 добавлено
            "editor": self.editor_choice.get(),
            "lang": self.lang.get(),     # 🟢 добавляем язык
            "playset_path": self.playset_path.get().strip(),
            "playset_enabled": self.playset_enabled.get(),
        }
        try:
    ...
    def _browse_localmods(self):
        """Диалог выбора папки локальных модов"""
        folder = filedialog.askdirectory(title=self.i18n("localmods"))
        if folder:
            self.localmods_entry.delete(0, tk.END)
            self.localmods_entry.insert(0, folder)
            self._save_config()

    def _browse_playset(self):
        """Выбор файла набора (Playset JSON)"""
        file_path = filedialog.askopenfilename(
            title=self.i18n("playset"),
            filetypes=[("JSON", "*.json"), ("All", "*.*")]
        )
        if file_path:
            self.playset_path.set(file_path)
            self._save_config()
    ...
    def _run_analysis(self):
        ...
        try:
            ...
            ws_path = Path(self.workshop_entry.get().strip())
            local_path = Path(self.localmods_entry.get().strip())

            paths_to_check = []
            if ws_path.exists():
                paths_to_check.append(ws_path)
            ...
            for mod_dir in paths_to_check:
                label = (self.i18n('workshop') if mod_dir == ws_path else self.i18n('localmods'))
                self._log(f"📂 {label}: {mod_dir}")
                mods_part = self._build_mod_structure(parsed, mod_dir)
                self.mod_errors.update(mods_part)
    ...
    def _parallel_classify_log(self, data: str):
        """
        Разбивает error.log на чанки и классифицирует их потоково (без процессов).
        Устраняет дубли, сохраняет порядок и работает быстрее.
        """
        ...
        # если лог маленький или многопоточность выключена
        if not getattr(self, "use_multithread", True) or total_lines < 2000:
            self._log("🧩 Single-thread classification mode")
            return self.classifier.classify_block(data)

        cpu_count = os.cpu_count() or 4
        workers = min(8, max(2, cpu_count))
        chunk_size = (total_lines + workers - 1) // workers
        self._log(f"🧵 Threaded classification: {workers} threads × {chunk_size} lines")

        # Разделяем без перекрытий
        chunks, offsets = [], []
        ...
        def classify_chunk(sub_lines, offset):
            """Классифицирует часть через существующий self.classifier"""
            out = []
            for j, line in enumerate(sub_lines, 1):
                parsed = self.classifier.classify_line(line)
                if parsed:
                    parsed.log_line = offset + j
                    out.append(parsed)
            return out

        try:
            with ThreadPoolExecutor(max_workers=workers) as exe:
                futures = [exe.submit(classify_chunk, c, o) for c, o in zip(chunks, offsets)]
                total = len(futures)
                for done, f in enumerate(as_completed(futures), 1):
                    try:
                        res = f.result()
                        if res:
                            results_tmp.extend(res)
                    except Exception as e:
                        self._log(f"⚠️ Chunk #{done} failed: {e}")
                    if done % 2 == 0 or done == total:
                        self.status_var.set(f"Classifying... {done}/{total}")
                        self.root.update_idletasks()
        except Exception as e:
            self._log(f"⚠️ Threaded classification failed: {e}")
            return self.classifier.classify_block(data)

        # Сортировка по строкам и устранение дублей
        results_tmp.sort(key=lambda e: e.log_line)
        unique, seen = [], set()
        for e in results_tmp:
            if e.log_line not in seen:
                seen.add(e.log_line)
                unique.append(e)

        self._log(f"✅ Classified {len(unique)} unique errors (from {total_lines} lines)")
        return unique
    # ──────────────────────────────── CORE STRUCTURE ────────────────────────────────

    def _build_mod_structure(self, parsed_errors, ws_path: Path):
        """
        Распределяет ошибки по структуре модов (Workshop + Local Mods).
        Делает точное сопоставление путей (common/...),
        добавляет реальную проверку кодировки для ENCODING_ERROR.
        Использует безопасную многопоточную индексацию модов.
        """
        ...
        # Вспомогательная проверка кодировки и BOM
        ...
        # безопасный лог из потоков
        def safe_log(msg: str):
            try:
                self.root.after(0, lambda: self._log(msg))
            except Exception:
                print(f"[ThreadLogFallback] {msg}")
        ...
        safe_log(self.i18n("workshop_index").format(total=total_mods))

        # ───────────────────────────────────────────
        # 🔹 Многопоточная индексация модов
        # ───────────────────────────────────────────
        lock = threading.Lock()

        def index_single_mod(mod_dir: Path):
            """Проходит один мод, возвращает info и словарь файлов."""
            try:
                ...
                for root, _, files in os.walk(mod_dir):
                    for f in files:
                        ...
                        try:
                            ...
                            continue
                return info, rel_index
            except Exception as e:
                safe_log(self.i18n("index_error").format(mod=mod_dir, err=e))
                return None, {}

        max_workers = max(2, min(8, (os.cpu_count() or 4)))
        safe_log(f"🧵 Using up to {max_workers} threads for mod indexing...")
        self.progress.start()

        if not getattr(self, "use_multithread", True):
            safe_log("ℹ️ Multithreading disabled in config.json → using single-thread mode.")
            for mod_dir in mod_dirs:
                info, rel_index = index_single_mod(mod_dir)
                if not info:
                    continue
                mods[info["id"]] = {
                    "id": info["id"], "name": info["name"],
                    "path": str(mod_dir), "errors": {}
                }
                file_index[info["id"]] = rel_index
            safe_log(self.i18n("index_done").format(count=len(file_index)))
            safe_log(self.i18n("process_errors").format(count=len(parsed_errors)))
            self.progress.stop()

        
        try:
            with ThreadPoolExecutor(max_workers=max_workers, thread_name_prefix="ck3mod") as executor:
                futures = {executor.submit(index_single_mod, d): d for d in mod_dirs}
                done_counter = 0
                for future in as_completed(futures):
                    mod_dir = futures[future]
                    try:
                        info, rel_index = future.result(timeout=120)
                        if not info:
                            continue
                        with lock:
                            mods[info["id"]] = {
                        ...
                        safe_log(f"⚠️ Timeout indexing {mod_dir}")
                    except Exception as e:
                        safe_log(f"⚠️ Worker error {mod_dir}: {e}")
                    finally:
                        done_counter += 1
                        if done_counter % 5 == 0 or done_counter == total_mods:
                            percent = (done_counter / total_mods) * 100
                            self.status_var.set(f"Indexing mods... {done_counter}/{total_mods} ({percent:.0f}%)")
                            self.root.update_idletasks()

        except Exception as e:
            safe_log(f"❌ Multithread indexing failed: {e}; falling back to single-thread mode.")
            for mod_dir in mod_dirs:
                try:
                    info, rel_index = index_single_mod(mod_dir)
                    if not info:
                    ...
                    safe_log(self.i18n("index_error").format(mod=mod_dir, err=e2))
            ...
            for rel_variant in possible_rel_keys:
                ...
                for mod_id, mod_info in mods.items():
                    ...
                    if candidate.exists():
                        ...
                        safe_log(self.i18n("match_exact").format(file=rel_variant, mod=mod_info["name"]))
                        break
                if found_in_mod:
                    break
            ...
            if not found_in_mod:
                for mod_id, rel_index in file_index.items():
                    if rel_key in rel_index:
                        ...
                        safe_log(self.i18n("match_indexed").format(file=rel_key, mod=mods[mod_id]["name"]))
                        break

            # 3️⃣ Проверка BOM‑ошибок
            if err.type == "ENCODING_ERROR":
                directory, filename = os.path.split(rel_key)
                same_named = [(mid, p) for mid, rels in file_index.items() for rel, p in rels.items() if rel.endswith("/" + filename)]
                if same_named:
                    safe_log(f"🔍 Найдено {len(same_named)} файлов '{filename}' в разных модах:")
                    bad_files = []
                    for mid, path_ in same_named:
                        ok, enc, has_bom = _check_bom_encoding(path_)
                        status = "OK" if ok else "BAD"
                        safe_log(f"   {mods[mid]['name']} → {path_.relative_to(ws_path)} | encoding={enc}, BOM={'yes' if has_bom else 'no'} → {status}")
                        if not ok:
                            bad_files.append((mid, path_, enc, has_bom))
                    if not bad_files:
                        safe_log(self.i18n("bom_all_ok").format(file=filename))
                        continue
                    for mid, path_, enc, has_bom in bad_files:
                        mark = "✅" if has_bom else "❌"
                        safe_log(f"{mark} {path_.name}: encoding={enc or 'n/a'}, BOM={'yes' if has_bom else 'no'} → {mods[mid]['name']}")
                        mod_info = mods[mid]
                        rel_path = str(path_.relative_to(mod_info["path"])).replace("\\", "/")
                        self._insert_mod_error(mod_info["errors"], rel_path, err)
                    continue

            # 4️⃣ Последнее — поиск по имени
            if not found_in_mod:
                ...
                for mod_id, rel_index in file_index.items():
                    for rel, p in rel_index.items():
                        if fnmatch.fnmatch(Path(rel).name.lower(), pattern.lower()):
                            ...
                            safe_log(self.i18n("match_loose").format(file=pattern, mod=mods[found_in_mod]["name"]))
                            break
                    if found_in_mod:
                        break
            ...
            if found_in_mod and found_path:
                try:
                    if err.type == "ENCODING_ERROR":
                        ...
                        mark = "✅" if ok else "❌"
                        safe_log(f"{mark} {found_path.name}: encoding={enc or 'n/a'}, BOM={'yes' if has_bom else 'no'} → {mods[found_in_mod]['name']}")
    ...
    def _check_mod_conflicts(self):
        """
        Проверка Workshop и локальных модов на конфликты и зависимости.
        Отображает древовидно: мод → файлы → другие моды.
        """
        t = self.i18n
        self.notebook.select(2)
        self.conf_tree.configure(show="tree headings")
        self.conf_tree.delete(*self.conf_tree.get_children())

        # 🟢 Сбор путей для анализа
        ws_path = Path(self.workshop_entry.get().strip())
        local_path = Path(self.localmods_entry.get().strip())
        paths_to_check = []
        if ws_path.exists():
        ...
        # ⚙️ Если активен фильтр по набору (Playset)
        playset_ids = set()
        if self.playset_enabled.get():
            playset_file = Path(self.playset_path.get().strip())
            if playset_file.exists():
                try:
                    with open(playset_file, "r", encoding="utf-8") as f:
                        data = json.load(f)
                    playset_ids = {m["steamId"] for m in data.get("mods", []) if m.get("enabled", True)}
                    self._log(self.i18n("playset_limited").format(count=len(playset_ids)))
                except Exception as e:
                    self._log(self.i18n("playset_load_error").format(err=e))
            else:
                self._log(self.i18n("playset_not_found"))

        self._log(self.i18n("check_conflicts_start"))
        duplicates: dict[str, list[str]] = {}
        mod_info: dict[str, dict] = {}

        # 🔁 Проходим по всем каталогам модов (Workshop + Local Mods)
        for base_path in paths_to_check:
            if not base_path.exists():
                continue

            mods = [d for d in base_path.iterdir() if d.is_dir()]
            # фильтрация по Playset, если задан
            if playset_ids:
                mods = [d for d in mods if any(d.name.endswith(mid) for mid in playset_ids)]
        ...
        for mid, info in sorted(mod_info.items(), key=lambda x: x[1]["name"].lower()):
            ...
            # replace_path из descriptor.mod
            ...
            for file_rel in sorted(info["files"]):
                ...
                if len(mod_list) > 1:  # конфликт ≥2 модов
                    ...
                    self.conf_tree.insert(
                        ...
                        values=(
                            "conflict",
                            t("others_count").format(count=len(others)),
                            ", ".join(others),
                            "",
                        ),
                    )
