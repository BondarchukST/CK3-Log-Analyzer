...
class CK3LogParser:
    ...
    def _setup_ui(self):
        ...
        self.playset_check.grid(row=2, column=0, sticky=tk.W)

        self.playset_entry = ttk.Entry(cfg, width=70, textvariable=self.playset_path)
        self.playset_entry.grid(row=2, column=1, padx=5)

        ttk.Button(cfg, text=t("browse_playset"), command=self._browse_playset).grid(row=2, column=2)

        editor_frame = ttk.LabelFrame(cfg, text=t("editor"), padding=(5, 2))
        editor_frame.grid(row=3, column=0, columnspan=3, sticky=tk.W, pady=(8, 0))
    ...
    def _check_mod_conflicts(self):
        ...
        if not ws_path.exists():
            ...
            return
        # ⚙️ Если включён фильтр по набору (Playset)
            ...
            if playset_file.exists():
                try:
                    with open(playset_file, "r", encoding="utf-8") as f:
                        data = json.load(f)
                    ids = {m["steamId"] for m in data.get("mods", []) if m.get("enabled", True)}
                    self._log(self.i18n("playset_limited").format(count=len(ids)))
                    # оставить в списке только моды из набора
                    self.mod_errors = {mid: mod for mid, mod in self.mod_errors.items() if mid in ids}
                except Exception as e:
        ...
        if self.playset_enabled.get() and locals().get("ids"):
            mods = [d for d in mods if any(d.name.endswith(sid) for sid in ids)]
